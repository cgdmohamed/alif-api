...
export class ReportsService {
  ...
  async overview(schoolId?: string) {
    ...
    const averageAttendance = attendanceRecords.length
      ? Math.round(
          (attendanceRecords.filter((a) => a.status === AttendanceStatus.PRESENT).length / attendanceRecords.length) *
            100,
        )
      : 0
  ...
  private async studentSummary(student: Student) {
    ...
    const average = graded.length ? Math.round(graded.reduce((sum, s) => sum + (s.grade ?? 0), 0) / graded.length) : 0

    const attendance = await this.attendanceRepository.find({
      where: { studentId: student.id },
    })
    const attendancePercent = attendance.length
      ? Math.round((attendance.filter((a) => a.status === AttendanceStatus.PRESENT).length / attendance.length) * 100)
      : 0
  ...
  async markAttendance(
    ...
    if (students.length !== uniqueStudentIds.length) {
      throw new ForbiddenException('Attendance contains a student outside this class')
    }
