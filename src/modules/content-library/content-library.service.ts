...
export class ContentLibraryService {
  constructor(
    @InjectRepository(ContentItem) private readonly itemsRepository: Repository<ContentItem>,
    @Inject(STORAGE_PROVIDER) private readonly storage: StorageProvider,
  ) {}

  findAll(filters: { folder?: string; type?: ContentItemType } = {}) {
    return this.itemsRepository.find({ where: filters, order: { createdAt: 'DESC' } })
  }
  ...
  async create(dto: CreateContentItemDto, file: { buffer: Buffer; originalname: string; size: number }) {
    ...
    return this.itemsRepository.save(
      this.itemsRepository.create({ ...dto, storageUrl, sizeBytes: file.size }),
    )
  }
